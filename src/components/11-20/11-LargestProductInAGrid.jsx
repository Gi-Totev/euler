...
const LargestProductInAGrid = ({ classes }) => {
  ...
  const getLargestProduct = () => {
    ...
    for (let i = 0; i < grid.length - 3; i++) {
      for (let j = 0; j < grid[i].length - 3; j++) {
        ...
        max = Math.max(max, horizontal, vertical, diagonal, oppositeDiagonal);
      }
    }
  ...
  return (
    <InputBox
      ...
      label2={null}
      input2={null}
      onClick={getLargestProduct}
      result={result}
      resultText={"Largest Product in the Grid:"}
    />
  );
};

export default LargestProductInAGrid;
